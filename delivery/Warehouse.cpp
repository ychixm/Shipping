...
void Warehouse::exportPath(unsigned seed) {
    ...
    std::cout << std::experimental::filesystem::current_path().u8string() << std::endl;

    file.open(std::experimental::filesystem::current_path().u8string() + "/results/"+ name +".txt",std::ios::app);
